...
update (Message (Just stanza)) state =
  onlyEffects state [ traceAnyA (toString stanza) >>= const (pure Nothing) ]
...
update (Connect jid password) state@{ connection, messageChannel, statusChannel } =
  { state: state { credentials = Just { jid, password } }
  , effects: [ spawnConnection ]
  }
 where
  spawnConnection = liftEff $ do
    traceAnyA connection
    c ← case connection of
      ...
      Nothing → do
        ...
        pure conn
    connect c statusChannel jid password
    pure <<< Just $ NewConnectionSpawned c
...
connect connection statusChannel jid password = do
  disconnect connection "new connection requested"
